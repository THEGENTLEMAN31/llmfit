...
- [x] **V2-e Moteur de placement** (implémentation minimale) — ✅ **TERMINÉ** (commit ade95e8) : nouveau `search_placements` (plan.rs) explore l'espace de dégradation `quant ↓ → ctx ↓ → offload ↑ → TP ↑` et retourne les candidats viables triés par score multi-objectif (qualité, vitesse, fit, contexte). Structure `PlacementCandidate` / `PlacementResult` ajoutée. Recherche actuelle : boucle sur quantifications, contextes (max → /2 jusqu'à 1k), modes (GPU, Serving, MoE/CPU offload, TP), n_cpu_moe/ngl. Utilise `fit::score_fit`, `compute_scores`, `weighted_score`, `estimate_tps` pour évaluer chaque candidat. MVP : candidat GPU de base si fit VRAM. Fallback OOM → chemin suivant. `CalcConfig` : champs serving/TP existants réutilisés. Tests : existants verts, structure sérialisable.
...
### 2026-08-24 — Session 4 (suite 4) — ✅ V2-e TERMINÉ (commit ade95e8)
- **Moteur de placement** (plan.rs, nouvelle fonction `search_placements`) : implémentation minimale (MVP) du moteur de recherche ordonnée sur l'échelle de dégradation `ctx ↓ → --n-cpu-moe ↑ → -ngl ↓` (roadmap V2-e). Types ajoutés : `PlacementCandidate` (mode, quant, ctx, TP, n_cpu_moe, n_gpu_layers, mémoire, TPS, score, commande) et `PlacementResult` (modèle, système, candidats triés par score, recommandation).
- **Algorithme MVP** : itération sur hiérarchie de quantification (meilleure → compressée), échelons de contexte (max → /2 jusqu'à 1k), tailles TP (1 → min(GPU count, 8)), modes d'exécution (GPU, Serving, MoeOffload, CpuOffload, TensorParallel, CpuOnly). Pour MoE : boucle n_cpu_moe 0..layers ; pour dense : échantillonnage ngl [99,64,32,16,8,4,2,1,0]. Évaluation par candidat : mémoire (V2-b reserves), débit (`fit::estimate_tps`), score multi-objectif (`fit::compute_scores` + `weighted_score`). Filtrage mémoire fit (tolérance 2 %). Génération commande `llamacpp_server_command` / `vllm serve`.
- **Intégration** : `CalcConfig` réutilise `serving_max_num_seqs`, `serving_paged_overhead_fraction`, `tensor_parallel_size`. Structures sérialisables (API JSON auto). Fallback OOM → chemin suivant. Cas UMA Apple : chemins GPU/CPUonly seulement.
- **Limites MVP connues** (documentées) : pas encore de branch-and-bound sur score, pas de Pareto-front explicite, facteurs TP/PP multi-GPU simplifiés (somme VRAM × TP), pas d'optimisation conjointe quant+ctx+offload, UMA Apple exclusif.
